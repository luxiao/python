...
template_path=r'D:\lux\work\template'
...
def main(task,task_type):
  ...
  if task_type=='boe':
    conf={'report':'WII127'
          ,'report_path':'CoreReport/CoreMonthly/Finance'
          ,'bv_path':'CoreReport/BV_WII127'
          ,'publish':'CoreMonthly_FIN'
          ,'publish_path':'CoreReport/CoreMonthly'
          ,'action':'CRT'}
  type_dir=template_path+'\\'+task_type+'\\'
  cfg_file=type_dir+task_type+'.cfg'
  tpl_dir=type_dir+'tpl\\'  
  conf['task']=task
  ...
  task_dir=os.path.join(conf['workdir'],task+r'\transfer')
...
def txt_rep(src,conf,tns='1'):
  ...
  if src.find('RTC')>=0:
    ...
    conf['rtc_type']='\\'+conf['task'][:conf['task'].index('2014')]
